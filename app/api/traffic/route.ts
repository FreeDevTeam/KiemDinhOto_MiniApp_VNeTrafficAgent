...
const API_URL = "https://ttdk-develop-server.service.makefamousapp.com"
...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(`${API_URL}${path}`, { method: "POST", headers: { apikey: API_KEY, ...(body.token ? { authorization: `Bearer ${body.token}` } : {}), "Content-Type": "application/json" }, body: JSON.stringify(body.action === "lookup" ? { plateNumber: body.plateNumber, vehicleType: body.vehicleType, phoneNumber: body.phoneNumber } : { username: body.username, ...(body.otp ? { otp: body.otp } : {}) }), cache: "no-store" })
